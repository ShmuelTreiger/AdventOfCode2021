...
for i in range(1, max(crab_positions.keys())):
    ...
    fuel_to_reach_point_i = fuel_to_reach_last_point + crabs_to_the_left - crabs_to_the_right
...
# starts point 0
fuel_to_reach_last_point = sum([(k * (k + 1) // 2) * v for k, v in crab_positions.items()])
for i in range(1, max(crab_positions.keys())):
